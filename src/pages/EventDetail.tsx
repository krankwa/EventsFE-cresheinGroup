...
import {
    ...
    Ticket, Loader2, CheckCircle2, XCircle,
} from "lucide-react";
import { eventsService } from "../services/eventsService";
import { registerTicket } from "../services/apiTickets";
import { useUser } from "../features/authentication/useUser";
import type { EventResponse, TicketTierResponse } from "../types/Event.types";
...
function TierRow({
    ...
    return (
        <div className={`rounded-xl border p-4 flex flex-col sm:flex-row sm:items-center gap-4 transition-colors
            ...
            <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2">
                    ...
                    {soldOut
                        ...
                        : <Badge variant="secondary" className="text-[10px]">{tier.availableTickets} left</Badge>}
                </div>
            ...
            <div className="flex items-center gap-4 sm:flex-col sm:items-end">
                ...
                >
                    {isBooking
                        ...
                        : soldOut ? "Sold Out" : <><Ticket className="w-3.5 h-3.5" />Book</>}
                </Button>
            </div>
        </div>
    );
...
export function EventDetail() {
    ...
    useEffect(() => {
        if (!id) return;
        eventsService.getById(Number(id))
            .then(setEvent)
    ...
    const handleBook = async (tierId: number) => {
        ...
        try {
            await registerTicket({ eventId: event!.eventID, tierId });
            ...
            <div className="relative w-full h-64 sm:h-80 rounded-2xl overflow-hidden bg-primary/10">
                ...
                {isSoldOut && (
                    <div className="absolute inset-0 bg-background/60 backdrop-blur-[2px] flex items-center justify-center">
                        <Badge variant="destructive" className="px-6 py-2 text-base font-bold uppercase tracking-wider">
                            Sold Out
                        </Badge>
                    </div>
                )}
