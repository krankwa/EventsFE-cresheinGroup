...
import { Calendar, MapPin, Users, ArrowRight, Loader2, Ticket, CalendarDays, Zap } from "lucide-react";
    ...
    <Card className="overflow-hidden group hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border-muted/40 bg-card/60 backdrop-blur-sm">
      ...
      <CardHeader className="px-5 pt-5 pb-2">
        <div className="flex justify-between items-start gap-2">
          ...
          <span className="text-primary font-bold text-sm whitespace-nowrap">₱999+</span>
        </div>
      </CardHeader>
      ...
      <CardContent className="px-5 pb-0 space-y-2">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="italic truncate">{format(new Date(event.date), "PPP")}</span>
        </div>
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
      ...
      <section id="events-section" className="container mx-auto px-4 pb-24">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Book your spot before they sell out</p>
          </div>
          {events.length > 0 && (
        ...
        {isLoading ? (
          ...
          <div className="py-24 text-center text-muted-foreground border-2 border-dashed rounded-3xl">
            ...
            <p className="text-sm mt-1">Check back soon for exciting upcoming events!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((event) => (
