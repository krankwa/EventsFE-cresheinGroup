...
export function UserDashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in mt-8 slide-in-from-bottom-4 duration-700">
      {/* Welcome Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight bg-blue-900 from-primary to-primary/60 bg-clip-text text-transparent">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/tickets">
            <Button className="rounded-full px-6 bg-blue-900 shadow-lg shadow-primary/20">
              My Tickets
            </Button>
          </Link>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2 overflow-hidden border-none shadow-xl bg-gradient-to-br from-primary/10 via-background to-background relative group">
          ...
          <CardHeader>
            ...
            <CardTitle className="text-2xl pb-1">
              {isLoading ? "Loading your next event..." : data?.nextEvent ? data.nextEvent.eventTitle : "No upcoming events"}
            </CardTitle>
            ...
            {data?.nextEvent ? (
              <div className="space-y-8">
                <div className="flex flex-wrap gap-4 md:gap-6 items-center">
                  <div className="bg-background/80 backdrop-blur-md p-6 rounded-2xl border shadow-sm flex-1 min-w-[200px] hover:bg-primary/[0.02] transition-colors">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground mb-2">When</p>
                    <p className="font-bold text-xl">{format(new Date(data.nextEvent.eventDate), "MMMM d, yyyy")}</p>
                    <p className="text-sm text-primary font-bold mt-1 uppercase tracking-wide">{format(new Date(data.nextEvent.eventDate), "p")}</p>
                  </div>
                  <div className="bg-background/80 backdrop-blur-md p-6 rounded-2xl border shadow-sm flex-1 min-w-[200px] hover:bg-primary/[0.02] transition-colors">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground mb-2">Seat / Tier</p>
                    <p className="font-bold text-xl">{data.nextEvent.tierName}</p>
                    <div className="mt-2 inline-flex items-center px-2 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-bold">Confirmed</div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-4 border-t border-primary/5">
                  <div className="flex items-center gap-3 px-4 py-2 rounded-full bg-primary/5 border border-primary/10">
                    ...
                    <div className="text-left">
                      <p className="text-xs font-bold leading-none">Shared Experience</p>
                      <p className="text-[10px] text-muted-foreground font-medium mt-1 uppercase tracking-tighter">Join <span className="text-primary font-bold">50+</span> others attending</p>
                    </div>
                  </div>
              ...
              <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div className="max-w-[280px]">
                  <p className="text-sm font-medium">You haven't booked any upcoming events yet.</p>
                  <p className="text-xs text-muted-foreground mt-1">Discover trending events nearby and secure your spot today!</p>
                </div>
                <Link to="/events">
                  <Button variant="default" className="rounded-full bg-blue-900">
                    Explore Events
                  </Button>
                </Link>
              </div>
        ...
        <Card className="h-full flex flex-col justify-between border-primary/10 shadow-lg group hover:border-primary/30 transition-colors overflow-hidden">
          <div className="p-6">
            {/* <div className="w-10 h-10 rounded-xl bg-violet-600/10 flex items-center justify-center text-violet-600 mb-4 group-hover:scale-110 transition-transform">
              <Sparkles className="w-5 h-5" />
            </div> */}
            <h3 className="text-lg font-bold mb-2">Pro Tip</h3>
            <p className="text-sm text-muted-foreground leading-relaxed">
              You can now add your tickets to Apple Wallet or Google Pay directly from the ticket details view.
              Make sure to have your QR code ready at the venue entrance.
            </p>
          </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Recommended based on your preferences</p>
          </div>
          <Link to="/events" className="text-primary text-sm font-bold flex items-center gap-1 hover:underline">
              ...
              <Link key={event.id} to={`/events/${event.id}`} className="group">
                <div className="relative aspect-[4/5] rounded-3xl overflow-hidden mb-3 shadow-md group-hover:shadow-2xl transition-all duration-500">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-6 flex flex-col justify-end transform translate-y-2 group-hover:translate-y-0 transition-transform">
                    <p className="text-white font-bold leading-tight group-hover:text-primary transition-colors">{event.title}</p>
